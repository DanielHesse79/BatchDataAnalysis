...
`docs/DOMAINS.md` records why the clinical half of this project moved to its own repository, [LabInsight](https://github.com/DanielHesse79/LabInsight), and the three words - QC, validation, batch - that mean different things on either side of that line. `docs/PRODUCT_SPLIT_PLAN.md` records how the split was done and what is left of it. `archive/` holds the clinical code as it was, excluded from the tests and the bundle, until the new product has proven itself.
